...
public:
    ...
    static void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
        Camera* cam = static_cast<Camera*>(glfwGetWindowUserPointer(window));
        cam->handleKeyCallback(window, key, scancode, action, mods);
    }
...
struct Ray{
    ...
    vec3 lensing(float mass, vec3 blackHole){
        vec3 OtoC = blackHole - origin;          // origin to pos
        float tClosest = dot(OtoC, direction);

        vec3 closestPoint = origin + direction * tClosest;
        vec3 diff = normalize(blackHole - closestPoint);
        float b = length(diff);

        float delta = 4.0 * G * mass / (c * c * b);
        vec3 newDir = normalize(direction + diff * delta);
        if(Gravlensing){
            return newDir;
        }
        return direction;
...
struct Object{
    ...
    Object(vec3 p, vec3 v, float r, Material m, float mass = 7.3 * pow(10, 19)) : position(p), velocity(v), radius(r), material(m), mass(mass) {}
    // raytracing
...
public:
    ...
    vec3 trace(Ray &ray, int depth = 0){
        ...
        for(auto& obj : objs){
            ...
            if(obj.Intersect(ray, t)){
                if(t < closest) {
                    closest = t;
                    hitObj = &obj;
                    vec3 OtoC = obj.position - ray.origin;          // origin to pos
                    float tClosest = dot(OtoC, ray.direction);

                    vec3 closestPoint = ray.origin + ray.direction * tClosest;
                    vec3 diff = normalize(obj.position - closestPoint);
                    float b = length(diff);
                    if(b < ((2 * obj.mass * G) / (c*c))) {return vec3(1.0f, 0.0f, 0.0f);}
                }
            }
            
        };

        if(!hitObj){return vec3(0.0f, 0.0f, 0.1f);}
        ...
        for(auto& light : lights) {
            ...
            vec3 lightDir = normalize(light->position - hitPoint);           // hitpoint to light
            ...
            float diff = dot(normal, lightDir);              // how light hits (0 = side, 1 = direct)
            float attenuation = 1.0f / (distanceToLight);
...
int main(){
    Engine engine;
    Scene scene;
    Camera camera(vec3(0.0f, 0.0f, 0.0f), -75.0f, -45.0f, 0.0f, 90.0f);
    camera.registerCallbacks(engine.window);
    float bhMass = 2 * pow(10, 30);
    float bhRad = (2.0f * G * bhMass) / (c*c);
    bhRad /= 1000;
    cout<<bhRad<<endl;
    scene.objs = {
                      // position          vel        radius   material:      color          spec   emis
        Object(vec3(0.0f, -5.0f, -19.0f), vec3(0.0f), 12.0f, Material(vec3(1.0f, 0.0f, 0.0f), 0.9f, 10.0f)),
        Object(vec3(20.0f, -2.0f, -11.0f), vec3(0.0f), 5.5f, Material(vec3(0.1f, 1.0f, 0.1f), 0.2f, 0.5f)),
        Object(vec3(-17.0f, -1.0f, -6.0f), vec3(0.0f), 7.0f, Material(vec3(0.1f, 0.1f, 1.0f), 0.8f, 0.3f)),
        Object(vec3(-17.0f, -10.0f, 9.0f), vec3(0.0f), 7.0f, Material(vec3(0.1f, 1.0f, 1.0f), 0.0f, 0.3f)),
        Object(vec3(20.0f, -10.0f, 20.0f), vec3(0.0f), 3.0f, Material(vec3(1.0f), 1.0f, 109.0f)),
        Object(vec3(-250.0f, 0.0f, 0.0f), vec3(0.0f, 0.0f, -0.0f), 52.0f, Material(vec3(0.1f, 0.0f, 0.01f), 0.9f, 130.0f)),
        // black hole
        Object(vec3(0.0f, 15.0f, 0.0f), vec3(0.0f), bhRad, Material(vec3(0.0f), 0.9f, 10.0f), bhMass),
    };
    // -- loop -- //
    ...
    while(!glfwWindowShouldClose(engine.window)){
        ...
        for(int y = 0; y < rHeight; ++y){
            for(int x = 0; x < rWidth; ++x){
                ...
                //cout<<"\n old dir: "<<ray.direction[0]<<", "<<ray.direction[1]<<", "<<ray.direction[2]<<endl;

                ray.direction = ray.lensing(2 * pow(10, 30), vec3(0.0f, 15.0f, 0.0f));
                ray.direction = normalize(ray.direction);
                //cout<<"dir: "<<ray.direction[0]<<", "<<ray.direction[1]<<", "<<ray.direction[2]<<endl;
                //cout<<"lense: "<<lense[0]<<", "<<lense[1]<<", "<<lense[2]<<endl;
                //cout<<"pos: "<<ray.origin[0]<<", "<<ray.origin[1]<<", "<<ray.origin[2]<<endl;
                //cout<<"new dir: "<<ray.direction[0]<<", "<<ray.direction[1]<<", \n"<<ray.direction[2]<<endl;
            ...
            for(auto& obj2 : scene.objs){
                if(&obj2 != &obj){
                    ...
                    if (distance > 0) {
                        ...
                        //obj.accelerate(acc[0] * deltaTime * pow(1, -15), acc[1] * deltaTime * pow(1, -15), acc[2] * deltaTime * pow(1, -15));
                        //collision
                    }
                }
            }
